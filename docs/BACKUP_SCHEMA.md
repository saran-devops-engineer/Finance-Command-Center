...
- Do **not** change `BACKUP_SIGNATURE`, `BACKUP_VERSION`, or `FinanceDataSnapshot.schemaVersion` without approval.
...
Domain types for nested objects live in `src/shared/domain/finance.ts`:
...
6. Validate `data.schemaVersion === 1`
...
2. Add migration in `backup-service.ts`
