...
export interface AnnualExtraSimulationView {
  isHomeLoan: boolean;
  valid: boolean;
  errors: string[];
  warnings: string[];
...
/**
 * Annual Extra Payment — ALWAYS reduces tenure (no strategy selection).
 ...
 */
export function simulateAnnualExtra(
  loan: Loan,
  annualExtraAmount: number,
  paymentMonth: number,
  debug = false
): AnnualExtraSimulationView | null {
  ...
  const result = homeLoanAmortizationEngine.simulateAnnualExtra({
    snapshot,
    annualExtraAmount,
    paymentMonth,
    debug
  ...
  return {
    ...
    annualExtraAmount: result.annualExtraAmount,
    paymentMonth: result.paymentMonth,
    annualPaymentsMade: result.annualPaymentsMade,
    ...
    outstandingReduction: result.totalExtraPaid,
